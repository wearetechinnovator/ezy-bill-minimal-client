...
const Quotation = ({ mode }) => {
  ...
  // Set quotation number prefix
  useEffect(() => {
    if (getBillPrefix?.length && !mode) {
      setFormData(prev => ({
        ...prev,
  ...
  */
  ...
  const saveBill = async () => {
    if (formData.party === "") {
      ...
      return toast("Please enter Estimate/Quotation Date", "error")
    }
    else if (formData.quotationNumber === "") {
      return toast("Please enter Estimate/Quotation Number", "error")
    }
    ...
    try {
      ...
      if (mode) {
        toast('Quotation update successfully', 'success');
        navigate(-1);
        return;
      }
