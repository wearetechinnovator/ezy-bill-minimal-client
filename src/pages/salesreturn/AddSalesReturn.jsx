  ...
  */
  ...
  const saveBill = async () => {
    ...
    try {
      ...
      if (mode === "edit") {
        toast('Sales Return update successfully', 'success');
        navigate(-1);
        return;
      }
  ...
  const getPartyInvoice = async (partyId) => {
    (async () => {
      try {
        const url = `${process.env.REACT_APP_API_URL}/salesinvoice/get-party-invoice`;
        const cookie = Cookies.get("token");
        const req = await fetch(url, {
          method: "POST",
          headers: {
        ...
        const data = res.map(d => ({ label: d.salesInvoiceNumber, value: d.salesInvoiceNumber }));
          ...
          <div className='content__body__main bg-white' id='addQuotationTable'>
            ...
            <div className='flex flex-col lg:flex-row items-center justify-around gap-4'>
              ...
              <div className='flex flex-col gap-2 w-full lg:w-1/3'>
                <p className='text-xs'>Select Sales Invoice <span className='required__text'>*</span></p>
                <SelectPicker
                  searchable={true}
                  data={partyInvoices}
                  ...
                  onChange={(v) => {
                    if (v) {
                      const selectedInvoice = allPartyInvoices.find(inv => inv.salesInvoiceNumber === v);
