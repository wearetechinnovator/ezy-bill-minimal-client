...
const PO = ({ mode }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    discountPercentage: '', finalAmount: '', autoRoundOff: false,
  ...
  */
  ...
  const saveBill = async () => {
    ...
    try {
      ...
      if (mode) {
        toast('PO update successfully', 'success');
        navigate(-1);
        return;
      }
                    ...
                    <tr key={i.QuotaionItem} className='border-b'>
                      ...
                      <td>
                        <div className='flex flex-col gap-2'>
                          <MySelect2
                            model={"item"}
                            onType={(v) => onItemChange(v, index, tax, ItemRows, setItemRows, setItems, 'purchase')}
